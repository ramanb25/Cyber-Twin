...
class chart
{
  ...
  function show($table,$group,$type)
  {
...
$months=array();
$values=array();
$query ="SELECT MONTH(end_time),count(end_time),YEAR(end_time) FROM `machine_failure` WHERE failed_unit='Clamping unit' group by MONTH(end_time)";
 $result=$db->query($query);
    $items= array();
     while ($row = $result->fetch_row()) {
      $i++;
...
function drawRightY() {
      var data = google.visualization.arrayToDataTable([
          ['Month', 'Feeding unit', 'Drilling unit','Clamping unit'],
